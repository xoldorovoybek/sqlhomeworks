...
(select firstname + ' ' + lastname as CustomerName from Customers)
 select CustomerName from cte where CustomerName like 'A%';
...
select employeeid, coalesce(firstname, lastname) from employees
...
select * from Customers order by City desc, postalcode desc
________________________________________
...
24. Use WHERE clause to filter for employees whose Age is less than 30 
and who work in either the 'HR' or 'IT' department.
...
26. Write a query that uses the ALL operator to find employees whose salary is greater than 
...
27. Write a query that filters the Orders table for orders placed in the last 180 days using BETWEEN and CURRENT_DATE. 
